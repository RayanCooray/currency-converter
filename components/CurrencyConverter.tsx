...
export default function CurrencyConverter() {
  ...
  return (
    <div className="min-h-screen p-6 ">
      ...
      <Card className="max-w-lg mx-auto shadow-xl border border-gray-700 bg-white/10 backdrop-blur-md rounded-xl p-6">
          ...
          <div className="flex gap-4">
            ...
            <Select onValueChange={setFrom} value={from}>
              ...
              <SelectContent className="bg-white/20 dark:bg-gray-800/30 backdrop-blur-md border border-white/40 dark:border-gray-700 rounded-lg shadow-lg">
                {countries.map((c) => (
                  <SelectItem key={c.code} value={c.code} className="">
                    {c.name}
                  </SelectItem>
                ))}
              </SelectContent>
      ...
      <div className="mt-6 max-w-lg mx-auto">
        ...
        <ul className="space-y-2">
          {history.map((transfer) => (
            <li key={transfer._id} className="flex justify-between items-center p-3 border border-white/40 dark:border-gray-700 rounded-lg shadow-md bg-gray-800/30 backdrop-blur-md ">
              <span>
                {transfer.amount} {transfer.fromCountry} → {transfer.convertedAmount} {transfer.toCountry}
              </span>
              <Button variant="destructive" size="sm" onClick={() => handleRevoke(transfer._id)}>
